# ============================================================================
# ASSESSMENT DETAIL CATEGORY VISUALIZATION MODULE
# For gmed package - reusable across apps
# Copy this to: /Users/fredbuckhold/Documents/GitHub/gmed/R/mod_assessment_detail_category.R
# ============================================================================

#' Assessment Detail Category Visualization UI
#'
#' Dynamic assessment visualization with category selection, charts, and filtered Plus/Delta feedback.
#' Organizes assessment data by rotation type (CC, Day Assessments, Consults, etc.) with interactive
#' category buttons and observation sub-types.
#'
#' @param id Module namespace
#' @param title Section title (default: "Detailed Assessment Analysis")
#' @export
mod_assessment_detail_category_ui <- function(id, title = "Detailed Assessment Analysis") {
  ns <- NS(id)
...
#' Assessment Detail Category Visualization Server
#'
#' @param id Module namespace
#' @param rdm_data Reactive containing assessment data (must have redcap_repeat_instrument column)
#' @param record_id Reactive containing resident record_id
#' @param data_dict Data dictionary (reactive or static)
#' @export
mod_assessment_detail_category_server <- function(id, rdm_data, record_id, data_dict) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns

    # Make data_dict reactive if it isn't already
    dict <- if (is.reactive(data_dict)) {
      data_dict
    } else {
      reactive({ data_dict })
    }
